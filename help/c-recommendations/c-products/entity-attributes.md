...
Verwenden Sie Entitätsattribute, um Produkt- oder Inhaltsinformationen an Recommendations zu übermitteln.
...
Recommendations sendet die `productId` oder `productPurchasedId` (`entity.id` im Code), die in den Algorithmen verwendet wird.

>[!NOTE]
>
>`entity.id` muss mit dem an die Bestellbestätigungsseite gesendeten `productPurchasedId` und den in Adobe Analytics-Produktberichten verwendeten `productId` übereinstimmen.
...
>[!NOTE]
>
>Wenn Sie at.js 2 verwenden.*x*, `mboxCreate` (wie im folgenden Beispiel verwendet) wird nicht mehr unterstützt. So geben Sie Produkt- oder Inhaltsinformationen mithilfe von at.js 2 an Recommendations weiter.*x*, verwenden Sie [targePageParams](/help/c-implementing-target/c-implementing-target-for-client-side-web/targetpageparams.md).
