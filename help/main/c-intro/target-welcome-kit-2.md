...
Bevor Sie mit [!DNL Adobe Target] beginnen, sollten Sie sich in dieser Übersicht mit der Lösung vertraut machen. In diesem Kapitel erfahren Sie mehr über die wichtigsten Funktionen der Lösung, die Touchpoints Ihrer Marke, die Implementierungsoptionen, wichtige Funktionen und Workflows der Benutzeroberfläche, Governance-Funktionen und die Rolle der Lösung im Gesamtwert von [!DNL Adobe Experience Cloud]. Sofern nicht mit einem [!DNL Adobe Target Premium]-Badge versehen, sind die in diesem Kapitel beschriebenen Funktionen sowohl in [!DNL Adobe Target Premium] als auch in [!DNL Adobe Target Standard] verfügbar. Weitere Informationen finden Sie unter [Einführung in Target](/help/main/c-intro/intro.md).

## Funktionen und Aktivitäten

Tests und Personalisierung sind die beiden umfassenden Funktionstypen, die [!DNL Target] bietet und die Sie beim Erstellen einer &quot;Aktivität&quot;in [!DNL Target] verwenden können. Möglicherweise wird der Begriff &quot;Test&quot;synonym mit &quot;Optimierung&quot;und &quot;Personalisierung&quot;synonym mit &quot;Zielgruppenbestimmung&quot;verwendet.
...
Weitere Informationen zu diesen Funktionen, deren Anwendungsbereichen und deren Verwendung finden Sie unter [Target-Aktivitätstypen](/help/main/c-activities/target-activities-guide.md).
...
| Erlebnis-Targeting (XT) | Stellen Sie einer bestimmten Zielgruppe Inhalte auf Basis benutzerdefinierter Regeln und Kriterien bereit. **[!UICONTROL Experience Targeting]** ist nützlich, um ein bestimmtes Erlebnis oder einen bestimmten Inhalt an eine bestimmte Zielgruppe zu richten, wenn Sie verstehen, dass eine Zielgruppe nützlich ist und einen guten Eindruck davon hat, welches Erlebnis bei ihnen ankommt. [Mehr Info](/help/main/c-activities/t-experience-target/experience-target.md). |
...
| Recommendations<br>(Premium) | Nutzen Sie die KI-Funktionen von Adobe Sensei für automatische Produkt- oder Inhaltsempfehlungen auf Basis früherer Aktivitäten des Besuchers oder anderer Kunden. [Mehr Info](/help/main/c-recommendations/recommendations.md). |
...
| IoT/An jedem Ort | [!DNL Target] bietet eine serverseitige Implementierung, sodass Sie dieselben Test- und Personalisierungsfunktionen in Aktivitäten verwenden können, die Sie auf Ihrer herkömmlichen Website, Ihrer mobilen Site und in mobilen Apps in E-Mails und auf Touchpoints verwenden, die keinen Browser aufweisen oder keinen JavaScript-Code verwenden. Sie können daher auch Kiosks, Set-Top-Boxen, Spielekonsolen, Sprachassistenten und andere nicht traditionelle Touchpoints testen und personalisieren. |

## Implementierungen

Viele von Ihnen möchten möglicherweise [!DNL Target] verwenden, um Ihre vielen verschiedenen digitalen Touchpoints zu testen und zu personalisieren, einschließlich herkömmlicher Web- und mobiler Touchpoints, aber auch Touchpoints, die keinen Browser aufweisen oder keinen JavaScript-Code verwenden. In einigen Fällen fordern interne oder externe Richtlinien zusätzliche Kontroll- und Sicherheitsstufen. Eventuell verwenden Sie auch Prozesse, die aus Leistungsgründen auf einem Backend-Server ausgeführt werden müssen. Diese Vielzahl an Einsatzmöglichkeiten unterstützt [!DNL Target] durch verschiedene Implementierungsmöglichkeiten: clientseitig, serverseitig oder als Hybridlösung.

| Implementierungstyp | Details |
| --- | --- |
| Clientseitig | Bei dieser Implementierung von [!DNL Target] stellt [!DNL Target] die mit einer Aktivität verbundenen Erlebnisse direkt auf dem Client-Browser bereit. Der Browser entscheidet, welches Erlebnis angezeigt werden soll, und zeigt es an. Auf Client-Seite können Sie einen WYSIWYG-Editor, den **[!UICONTROL Visual Experience Composer]** (VEC) oder eine nicht visuelle Schnittstelle, den **[!UICONTROL Form-based Experience Composer]**, verwenden, um Ihre Test- und Personalisierungserlebnisse zu erstellen. [Weitere Infos](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/at-js/how-atjs-works.html){target=_blank}. |
...
[!DNL Target] bietet drei Hauptmethoden zum Einrichten Ihrer Test- und Personalisierungsaktivitäten: den [!UICONTROL Visual Experience Composer] (VEC), den [!UICONTROL Form-based Experience Composer] und den [!UICONTROL Single Page Application (SPA) Visual Experience Composer]. Beide führen Sie durch den Aktivitäts-Einrichtungsprozess in drei Schritten: Definieren der Erlebnisse, Auswählen oder Definieren der Zielgruppen und Auswählen der primären und sekundären Erfolgsmetriken, anhand derer Sie die Ergebnisse Ihrer Aktivität messen.
...
Für die Zuweisung der richtigen Benutzerrollen, Zugriffsebenen und Berechtigungen für [!DNL Target] verwenden Sie die Verwaltungskonsole. Für Benutzer von [!UICONTROL Target Premium] bieten wir eine detailliertere Verwaltung und Kontrolle
...
| Target-APIs (Premium) | [!UICONTROL Target] bietet mehr als 40 APIs, mit denen Sie Adobe Target in Erstanbieter-, Zweit- und Drittanbietersysteme integrieren können. [Mehr Info](/help/main/api/api-overview.md). |
