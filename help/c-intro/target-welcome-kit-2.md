...
Viele von Ihnen möchten [!DNL Target] möglicherweise verwenden, um Ihre vielen verschiedenen digitalen Touchpoints zu testen und zu personalisieren, einschließlich herkömmlicher Web- und Mobilgeräte-Touchpoints, aber auch Touchpoints, bei denen kein Browser oder kein JavaScript-Code verwendet wird. In einigen Fällen erfordert die interne oder externe Politik zusätzliche Kontroll- und Sicherheitsniveaus. Es können auch Prozesse vorhanden sein, die aus Leistungsgründen auf einem Backend-Server ausgeführt werden müssen. Um diese Vielzahl von Einsatzmöglichkeiten zu erfüllen, bieten wir Ihnen die Möglichkeit, [!DNL Target] auf verschiedene Weise zu implementieren: clientseitig, serverseitig oder eine Kombination aus beiden.

| Implementierungstyp | Details |
| --- | --- |
| Client-seitig | Bei dieser Implementierung von [!DNL Target] liefert [!DNL Target] die Erlebnisse, die einer Aktivität zugeordnet sind, direkt an den Clientbrowser. Der Browser entscheidet, welches Erlebnis angezeigt werden soll, und zeigt es an. Clientseitig können Sie einen WYSIWYG-Editor, den **[!UICONTROL Visual Experience Composer]** (VEC) oder eine nicht visuelle Schnittstelle, den **[!UICONTROL Form-Based Experience Composer]**, verwenden, um Ihre Test- und Personalisierungserfahrungen zu erstellen. [Mehr Info](/help/c-implementing-target/c-implementing-target-for-client-side-web/c-how-atjs-works/how-atjs-works.md). |
| Serverseitig | Bei dieser Implementierung von [!DNL Target] stellt ein Client-Gerät über Ihren Server eine Anforderung für ein Erlebnis auf, Ihr Server sendet diese Anforderung an [!DNL Target], [!DNL Target] sendet die Antwort an Ihren Server zurück und Ihr Server entscheidet, welches Erlebnis an das Client-Gerät gesendet werden soll, damit es wiedergegeben werden kann. Das Erlebnis muss nicht in einem Browser angezeigt werden. Es kann auch in einer E-Mail oder an einem Kiosk, über einen Sprachassistenten oder über ein anderes nicht visuelles Erlebnis oder nicht browserbasiertes Gerät angezeigt werden. Da sich Ihr Server zwischen dem Client und [!DNL Target] befindet, ist diese Art der Implementierung auch dann optimal, wenn Sie mehr Kontrolle und Sicherheit benötigen oder komplexe Backend-Prozesse haben, die auf Ihrem Server ausgeführt werden sollen. [Mehr Info](/help/c-implementing-target/c-api-and-sdk-overview/api-and-sdk-overview.md). |
