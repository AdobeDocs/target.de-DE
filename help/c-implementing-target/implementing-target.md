---
description: Bevor Sie mit der Verwendung von Adobe Target beginnen, sollten Sie Target auf Ihrer Site implementieren, sich mit einigen Grundkonzepten und Begriffen sowie mit der Funktionsweise von Target und mit dessen Integration in Ihre Infrastruktur vertraut machen und verstehen, wie Besucher durch das Target-System verfolgt werden.
keywords: Implementieren; wird implementiert
seo-description: Bevor Sie mit der Verwendung von Adobe Target beginnen, sollten Sie Target auf Ihrer Site implementieren, sich mit einigen Grundkonzepten und Begriffen sowie mit der Funktionsweise von Target und mit dessen Integration in Ihre Infrastruktur vertraut machen und verstehen, wie Besucher durch das Target-System verfolgt werden.
seo-title: Target implementieren
solution: Target
subtopic: Erste Schritte
title: Target implementieren
topic: Standard
uuid: 08482171-73c0-4816-bf4d-4571d92f04ab
translation-type: tm+mt
source-git-commit: 3bfed90752235909ed0e19dab460a4ec1a16dc78

---


# Target implementieren{#implement-target}

Bevor Sie mit der Verwendung von Adobe Target beginnen, sollten Sie Target auf Ihrer Site implementieren, sich mit einigen Grundkonzepten und Begriffen sowie mit der Funktionsweise von Target und mit dessen Integration in Ihre Infrastruktur vertraut machen und verstehen, wie Besucher durch das Target-System verfolgt werden.

+ [Vor der Implementierung](c-considerations-before-you-implement-target/considerations-before-you-implement-target.md)
+ [Clientseitige Webimplementierung](c-implementing-target-for-client-side-web/c-how-atjs-works/how-atjs-works.md)
+ [Implementierung von Apps für Mobilgeräte](/help/c-target-mobile-app/target-mobile-app.md)
+ [Serverseitige Implementierung](c-api-and-sdk-overview/target-api-documentation.md)
+ [E-Mail-Implementierung](c-non-javascript-based-implementation/non-javascript-based-implementation.md)
